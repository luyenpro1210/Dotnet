...
import { GET_ALL_CATEGORIES, GET_PRODUCT_ID, POST_ADD_CATEGORY, PUT_EDIT_CATEGORY, postCategory } from '../api/apiService'; // Import your API service function for adding a category
...
export default function CategoryEdit() {
    ...
    useEffect(() => {
        GET_PRODUCT_ID(`Categories`, id).then(item => {
            console.log(item)
            setCategoryData({
                parentId: item.data.parentId,
                categoryName: item.data.categoryName,
                categoryDescription: item.data.categoryDescription,
                icon: item.data.icon,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await PUT_EDIT_CATEGORY(id, formData);
                                ...
                                <Paper>
                                    <form onSubmit={handleSubmit}>
                                        ...
                                        <Grid item xs={12}>
                                            ...
                                            {categoryData.image && (
                                                <Grid item xs={12}>
                                                    <img src={`data:image/svg+xml;base64,${categoryData.image}`} alt={categoryData.categoryName} width={100} height={100} className="menu-title-img" />
                                                </Grid>
                                            )}
                                        </Grid>
